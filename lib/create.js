...
function copyJsAndLibrary (projectPath, shared, projectName, targetAPI) {
    ...
    fsExtra.copySync(srcCordovaJsPath, path.join(app_path, 'assets', 'www', 'cordova.js'));

    // Copy the cordova.js file to platforms/<platform>/platform_www/
    // The www dir is nuked on each prepare so we keep cordova.js in platform_www
    fs.mkdirSync(platform_www, { recursive: true });
    fsExtra.copySync(srcCordovaJsPath, path.join(platform_www, 'cordova.js'));
    fsExtra.copySync(path.join(ROOT, 'framework', 'cdv-gradle-config-defaults.json'), path.join(projectPath, 'cdv-gradle-config.json'));
    ...
    if (shared) {
        ...
        fsExtra.copySync(path.join(ROOT, 'framework', 'AndroidManifest.xml'), path.join(nestedCordovaLibPath, 'AndroidManifest.xml'));
        ...
        fsExtra.copySync(path.join(ROOT, 'framework', 'build.gradle'), path.join(nestedCordovaLibPath, 'build.gradle'));
        fsExtra.copySync(path.join(ROOT, 'framework', 'cordova.gradle'), path.join(nestedCordovaLibPath, 'cordova.gradle'));
        fsExtra.copySync(path.join(ROOT, 'framework', 'repositories.gradle'), path.join(nestedCordovaLibPath, 'repositories.gradle'));
        fsExtra.copySync(path.join(ROOT, 'framework', 'src'), path.join(nestedCordovaLibPath, 'src'), { recursive: true });
    }
}
...
function copyBuildRules (projectPath) {
    ...
    fsExtra.copySync(path.join(srcDir, 'build.gradle'), path.join(projectPath, 'build.gradle'));
    fsExtra.copySync(path.join(srcDir, 'app', 'build.gradle'), path.join(projectPath, 'app', 'build.gradle'));
    fsExtra.copySync(path.join(srcDir, 'app', 'repositories.gradle'), path.join(projectPath, 'app', 'repositories.gradle'));
    fsExtra.copySync(path.join(srcDir, 'repositories.gradle'), path.join(projectPath, 'repositories.gradle'));
...
function copyScripts (projectPath) {
    ...
    fsExtra.copySync(srcScriptsDir, destScriptsDir, { recursive: true });
}
...
function copyGradleTools (projectPath) {
    ...
    fsExtra.copySync(path.resolve(srcDir, 'tools'), path.resolve(projectPath, 'tools'), { recursive: true });
}
...
exports.create = function (project_path, config, options, events) {
    ...
    return exports.validatePackageName(package_name)
        .then(function () {
            ...
            fsExtra.copySync(path.join(project_template_dir, 'assets'), path.join(app_path, 'assets'), { recursive: true });
            fsExtra.copySync(path.join(project_template_dir, 'res'), path.join(app_path, 'res'), { recursive: true });
            fsExtra.copySync(path.join(project_template_dir, 'gitignore'), path.join(project_path, '.gitignore'));
            ...
            fsExtra.copySync(path.join(project_template_dir, 'Activity.java'), activity_path);
